...
class GameMaster:
    ...
    def hack_menu(self):
        ...
        print(
            f"Vulnerabilities left to exploit: {self.decryption_tasks_left} decryption, {self.intrusion_tasks_left} intrusion, {self.sys_manipulation_tasks_left} sys-manipulation")
        time.sleep(1)
        print()
        if self.on_corrupt:
            print("<!!!!!!!Corrupt Server Found!!!!!!!->")
            time.sleep(1)
            print("2: Get 1 decryption script", "3: Get 1 intrusion script", "4: Get 1 sys script",
                  "5: Get 1 antivirus protection")
            while True:
            ...
            time.sleep(1)
            print("1: Enter full decryption key to portal")
            while True:
                action = self.input_handler.handle_int_input("Please enter desired action:")
                if action == 1:
                    ...
                    if self.game_won:
                        ...
                        print("You failed to decrypt the AI, try again later")
                        return "Portal Fail"
                else:
                    print("Invalid action")
